...
public class ScoreboardManager {
    public void createScoreboard(Player player) {
        ...
        FrameAnimatedString fas = new FrameAnimatedString(Main.plugin().getConfig().getStringList("scoreboard.title"));

        new BukkitRunnable() {
            @Override
            public void run() {
                board.updateTitle(fas.next());
                board.updateLines(Main.plugin().getConfig().getStringList("scoreboard.content"));
